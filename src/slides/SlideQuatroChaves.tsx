  ...
  const cards = [
    {
      ...
      color: "bg-rose-600 hover:bg-rose-700 border-rose-700",
      textColor: "text-white",
      modalColor: "bg-rose-600"
    },
    {
    ...
    },
    {
      ...
      source: "Lubianca Neto et al., 2022 · Braz J Otorhinolaryngol · PMID 33472759",
      color: "bg-slate-800 hover:bg-slate-900 border-slate-900",
      textColor: "text-white",
      modalColor: "bg-slate-800"
    },
    {
      ...
      <div className="grid grid-cols-2 gap-3 lg:gap-5 w-full max-w-5xl mx-auto shrink-0 px-2">
        {cards.map((c, i) => (
          <Reveal show={step >= i + 2} key={c.title}>
            ...
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center font-black text-xl mb-3 bg-white/20 text-white">
                {i + 1}
              </div>
              <h3 className={`text-lg lg:text-2xl font-black tracking-tight ${c.textColor}`}>{c.title}</h3>
              <div className="mt-4 rounded-full p-2 flex items-center justify-center w-8 h-8 mx-auto bg-white/20 text-white">
        ...
        {activeCard !== null && (
          <div className="flex flex-col h-full bg-white relative">
            <div className="p-8 border-b border-slate-100 flex items-center gap-5">
              <div className={`w-16 h-16 text-white rounded-[1.5rem] flex items-center justify-center font-black text-3xl shadow-md ${cards[activeCard].modalColor}`}>
                {activeCard + 1}
              </div>
